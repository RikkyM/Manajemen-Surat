    ...
    <main class="flex h-screen overflow-y-auto bg-gray-100">
        ...
        <div class="h-full flex-1 overflow-y-auto p-8">
            <div class="grid gap-2">
                <h2 class="text-2xl font-semibold">Surat Keterangan Kematian</h2>
                <span><a href="{{ route('admin.dashboard') }}">Dashboard</a> > <a href="{{ route('admin.domisili') }}">Surat
                        Keterangan
                        Kematian</a> > <span class="font-semibold">Detail Surat</span></span>
            </div>
            <div class="flex">
                <div class="mt-3 grid flex-1 grid-cols-2 gap-5 rounded-md bg-white p-5">
                    ...
                    <div class="font-semibold">
                        <h2>Tempat Meninggal</h2>
                        <span>{{ $data->tempat_meninggal }}</span>
                    </div>
                    <div class="font-semibold">
                        <h2>Tanggal Meninggal</h2>
                        <span>{{ $data->tanggal_meninggal }}</span>
                    </div>
                    <div class="font-semibold">
                        <h2>Waktu Meninggal</h2>
                        <span>{{ $data->waktu_meninggal }}</span>
                    </div>
                    <div class="font-semibold">
                        <h2>Pekerjaan</h2>
                ...
                <div class="mt-3 overflow-y-auto rounded-xl border-2 border-black">
                    <embed src="{{ asset('upload_kk/skk/' . $data->upload_kk) }}" class="h-full w-[500px]"
    ...
    <script>
        ...
        statusBtn.addEventListener('click', () => {
            ...
            if (confirmation == true) {
                fetch(`/api/statusSkk/${statusBtn.dataset.id}`, {
                    method: "POST",
                    headers: {
