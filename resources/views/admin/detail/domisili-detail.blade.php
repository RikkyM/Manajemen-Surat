...
@section('pages')
    <main class="flex h-screen overflow-y-auto bg-gray-100">
        ...
        <div class="h-full flex-1 overflow-y-auto p-8">
            <div class="grid gap-2">
                <h2 class="text-2xl font-semibold">Surat Keterangan Domisili</h2>
                <span><a href="{{ route('admin.dashboard') }}">Dashboard</a> > <a href="{{ route('admin.domisili') }}">Surat
                        Keterangan
                        Domisili</a> > <span class="font-semibold">Detail Surat</span></span>
            </div>
            <div class="flex">
                <div class="mt-3 grid flex-1 grid-cols-2 gap-5 rounded-md p-5">
                ...
                <div class="mt-3 overflow-y-auto border-2 border-black rounded-xl">
                    <embed src="{{ asset('upload_kk/skd/' . $data->upload_kk) }}" class="h-full w-[500px]"
    ...
    <script>
        ...
        // const konfirmasi = document.querySelector('#konfirmasiBtn')
        ...
        statusBtn.addEventListener('click', () => {
            ...
            if (confirmation == true) {
                fetch(`/api/statusSkd/${statusBtn.dataset.id}`, {
                    method: "POST",
                    headers: {
        ...
        //     const konfirm = confirm('Lanjut untuk konfirmasi pengajuan ?')
        //     if (konfirm == true) {
        //         fetch(`/api/konfirmasiSkd/${konfirmasi.dataset.id}`, {
        //             method: "POST",
        //             headers: {
        //                 'Content-Type': 'application/json',
        //                 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')
