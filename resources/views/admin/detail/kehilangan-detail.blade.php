    ...
    <main class="flex h-screen overflow-y-auto bg-gray-100">
        ...
        <div class="h-full flex-1 overflow-y-auto p-8">
            <div class="grid gap-2">
                <h2 class="text-2xl font-semibold">Surat Keterangan Kehilangan</h2>
                <span><a href="{{ route('admin.dashboard') }}">Dashboard</a> > <a href="{{ route('admin.domisili') }}">Surat
                        Keterangan
                        Kehilangan</a> > <span class="font-semibold">Detail Surat</span></span>
            </div>
            <div class="flex">
                ...
                <div class="mt-3 overflow-y-auto border-2 border-black rounded-xl">
                    <embed src="{{ asset('upload_kk/skh/' . $data->upload_kk) }}" class="h-full w-[500px]"
    ...
    <script>
        ...
        statusBtn.addEventListener('click', () => {
            ...
            if (confirmation == true) {
                fetch(`/api/statusSkh/${statusBtn.dataset.id}`, {
                    method: "POST",
                    headers: {
