            ...
            @if ($skk->count() > 0)
                @foreach ($skk as $item)
                    <tr class="border hover:bg-slate-100">
                        ...
                        <td class="border py-3">Surat Keterangan Kematian</td>
                        ...
                        <td>
                            <div class="flex h-full items-center justify-center gap-2">
                                <buttton class="cursor-pointer bg-teal-500 px-2 py-1 text-white" id="skkBtn"
                                    data-id="{{ $item->id }}">
                                    Lihat
                                </buttton>
                                @if ($item->status == 1)
                                    <a href="{{ route('user.skkPdf', ['id' => $item->id]) }}"
...
<script>
    document.querySelectorAll('#skkBtn').forEach(button => {
        button.addEventListener('click', function() {
            fetch(`/api/getSkk/${this.getAttribute('data-id')}`)
                .then(response => {
                    return response.json()
                })
                .then(response => {
                    const modalSkkContainer = document.querySelector('#modal');
                    const modalSkk = document.querySelector('#modalBody');
                    modalSkkContainer.classList.remove('hidden');
                    ...
                    const tanggalMeninggal = new Date(response.data.tanggal_meninggal);
                    const tanggalM = tanggalMeninggal.getDate();
                    const bulanM = tanggalMeninggal.getMonth() + 1;
                    const tahunM = tanggalMeninggal.getFullYear();
                    const tanggalFormatM = tanggalM < 10 ? '0' + tanggalM : tanggalM;
                    const bulanFormatM = bulanM < 10 ? '0' + bulanM : bulanM;
                    const tahunFormatM = tahunM.toString();
                    const tanggalMeninggalFormat =
                        `${tanggalFormatM}-${bulanFormatM}-${tahunFormatM}`;

                    modalSkk.innerHTML = `
                        <h2 class="text-center text-xl font-bold">Detail Surat Kematian</h2>
                        <table>
                            <tr>
                            ...
                            <tr>
                                <td class="p-2">Tempat Meninggal</td>
                                <td class="p-2">: ${response.data.tempat_meninggal}</td>
                            </tr>
                            <tr>
                                <td class="p-2">Tanggal Meninggal</td>
                                <td class="p-2">: ${tanggalMeninggalFormat}</td>
                            </tr>
                            <tr>
                                <td class="p-2">Waktu Meninggal</td>
                                <td class="p-2">: ${response.data.waktu_meninggal}</td>
                            </tr>
                            <tr>
                    ...
                    document.getElementById('closeSkd').addEventListener('click', function() {
                        modalSkkContainer.classList.add('hidden');
                    });
                });
        });
    });
