            ...
            @if ($skd->count() > 0)
                @foreach ($skd as $item)
                    <tr class="border hover:bg-slate-100">
                        ...
                        <td class="border py-3">Surat Keterangan Domisili</td>
                        ...
                        <td>
                            <div class="flex h-full items-center justify-center gap-2">
                                <buttton class="cursor-pointer bg-teal-500 px-2 py-1 text-white" id="skdBtn"
                                    data-id="{{ $item->id }}">
                                    Lihat
                                </buttton>
                                @if ($item->status == 1)
                                    <a href="{{ route('user.skdPdf', ['id' => $item->id]) }}"
...
<script>
    document.querySelectorAll('#skdBtn').forEach(button => {
        button.addEventListener('click', function() {
            fetch(`/api/getSkd/${this.getAttribute('data-id')}`)
                .then(response => {
                    return response.json()
                })
                .then(response => {
                    const modalSkdContainer = document.querySelector('#modal');
                    const modalSkd = document.querySelector('#modalBody');
                    modalSkdContainer.classList.remove('hidden')
                    ...
                    modalSkd.innerHTML = `
                        <h2 class="text-center text-xl font-bold">Detail Surat Domisili</h2>
                         <table>
                            <tr>
                            ...
                            <tr>
                                <td class="p-2">Kewarganegaraan</td>
                                <td class="p-2">: ${response.data.kewarganegaraan}</td>
                            </tr>
                            <tr>
                    ...
                    document.getElementById('closeSkd').addEventListener('click', function() {
                        modalSkdContainer.classList.add('hidden');
                    });
                });
        });
    });
