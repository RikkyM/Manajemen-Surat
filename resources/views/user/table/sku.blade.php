            ...
            @if ($sku->count() > 0)
                @foreach ($sku as $item)
                    <tr class="border hover:bg-slate-100">
                        ...
                        <td class="border py-3">Surat Keterangan Usaha</td>
                        ...
                        <td>
                            <div class="flex h-full items-center justify-center gap-2">
                                <buttton class="cursor-pointer bg-teal-500 px-2 py-1 text-white" id="skuBtn"
                                    data-id="{{ $item->id }}">
                                    Lihat
                                </buttton>
                                @if ($item->status == 1)
                                    <a href="{{ route('user.skuPdf', ['id' => $item->id]) }}"
...
<script>
    document.querySelectorAll('#skuBtn').forEach(button => {
        button.addEventListener('click', function() {
            fetch(`/api/getSku/${this.getAttribute('data-id')}`)
                .then(response => {
                    return response.json()
                })
                .then(response => {
                    const modalSkuContainer = document.querySelector('#modal');
                    const modalSku = document.querySelector('#modalBody');
                    modalSkuContainer.classList.remove('hidden')
                    ...
                    modalSku.innerHTML = `
                        <h2 class="text-center text-xl font-bold">Detail Surat Usaha</h2>
                        <table>
                            <tr>
                    ...
                    document.getElementById('closeSkd').addEventListener('click', function() {
                        modalSkuContainer.classList.add('hidden');
                    });
                });
        });
    });
